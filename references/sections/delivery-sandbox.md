...
# B2C Дверь-Терминал сценарий
Дверь-Терминал сценарий называется так, потому что посылка "едет" от двери отправителя до терминала (ПВЗ) получателя.
Забор посылки от двери отправителя осуществляется курьером СД.

Сценарий Дверь-Терминал пока есть только в B2C: отправителем выступает юридическое лицо, которое вместо самостоятельной
сдачи КГТ посылки в терминал вызывает курьера службы доставки.

> [!IMPORTANT]
> В этой схеме для одного Avito заказа может быть зарегистрировано несколько посылок в службе доставки, одна из которых действительно поедет. Первоначально при оформлении заказа создается посылка B2C-ПВЗ.
> - Если отправитель вызывает курьера на UI, создается посылка Дверь-Терминал, которая может быть использована далее вместо посылки B2C-ПВЗ для движения.
> - Если вызов курьера будет отменен, мы отменим посылку Дверь-Терминал - отправитель может вызвать курьера для этого же заказа еще 1 раз или же отнести посылку на ПВЗ самостоятельно (в таком случае будет задействована посылка B2C-ПВЗ).
> - Если курьер успешно заберет посылку у курьера - будет отменена посылка B2C-ПВЗ, так как дальше идет работа только с посылкой Дверь-Терминал.


С точки зрения содержимого запрос на создание Дверь-Терминал посылки отличается от запроса на создание B2C-ПВЗ посылки
тем, что к отправителю за посылкой приедет курьер. Детали курьерского забора будут в поле `sender.delivery.courier`:

- `options.deliveryType` – всегда `DELIVERY_FROM_DOOR` (забор от двери).
- `options.comment` - комментарий для курьера.
- `options.elevatorAvailable` - наличие лифта по адресу забора.
- `pickupContact` – контактное лицо, у которого курьер заберет посылку. Может отличаться от `sender.name`.
- `dateTimeInterval` – интервал приезда курьера за посылкой.
- `address` - детали адреса забора посылки.

### Примеры запросов на создание посылок

<details>
<summary>Пример запроса на создание B2C Дверь-Терминал посылки</summary>
  ...
  "items": [
    {
      ...
      "description": "Покрасочная камера в отличном состоянии.",
      "cost": 3000000,
      "quantity": 1,
      "breadcrumbs": [
        {
    ...
    "delivery": {
      ...
      "courier": {
        ...
        "dateTimeInterval": {
          ...
          "end": "2026-07-01T13:00:00+03:00"
        },
        "options": {
          "deliveryType": "DELIVERY_FROM_DOOR",
          "deliveryConfirmationType": "PHONE",
          "elevatorAvailable": true,
...
### Схема движения Дверь-Терминал посылок
Действующие лица:
- Покупатель, он же получатель.
- Продавец (юридическое лицо), он же отправитель.
- Avito.
- Служба доставки (3PL).
- Курьер 3PL.
...
1. Продавец вызывает курьера для забора посылки. Avito создает посылку в службе доставки 3PL.
2. Служба доставки 3PL назначает курьера на забор посылки.
3. Курьер 3PL приезжает к продавцу в согласованный интервал и забирает посылку PARCEL.
4. Курьер 3PL сдает посылку PARCEL в терминал 3PL terminal 1.
5. Служба доставки 3PL доставляет посылку в терминал 3PL terminal 2 в городе получателя.
6. Получатель приходит в терминал 3PL terminal 2 и забирает посылку PARCEL.

После сдачи посылки курьером в терминал движение и трекинг посылки не отличаются от
[B2C-ПВЗ сценария](https://developers.avito.ru/api-catalog/delivery-sandbox/documentation#info/b2c-pvz_scenariy).

Итого, для реализации сценария Дверь-Терминал нужно поддержать работу с Avito по нескольким методам API:
- [Создание посылки](https://developers.avito.ru/api-catalog/delivery-sandbox/documentation#operation/createParcel)
  ([описание](https://developers.avito.ru/api-catalog/delivery-sandbox/documentation#info/sozdanie_posylki)).
- [Трекинг](https://developers.avito.ru/api-catalog/delivery-sandbox/documentation#info/treking)
  ([описание](https://developers.avito.ru/api-catalog/delivery-sandbox/documentation#info/treking)).
...
### Трекинг Дверь-Терминал посылок
На этапе курьерского забора ожидаем от службы доставки треки:

#### Успешный флоу

1. `CONFIRMED` \+ `COURIER_ASSIGNED` – курьер назначен на забор посылки.
2. `IN_TRANSIT` \+ `PARCEL_IS_TAKEN_BY_COURIER_FOR_INTAKE` – курьер забрал посылку у отправителя.
3. `IN_TRANSIT` \+ `RECEIVED_ON_SENDER_TERMINAL` – курьер сдал посылку в терминал.

Дальше посылка трекается так же, как в B2C-ПВЗ сценарии.

#### Негативные сценарии
1. Не удалось назначить курьера.
Мы будем ожидать событие трекинга:
- `CONFIRMED` \+ `COURIER_NOT_FOUND`

На нашей стороне у нас будет таймер (на начало интервала забора), до этого момента должно поступить событие трекинга о назначении курьера (`CONFIRMED` + `COURIER_ASSIGNED`). Если курьер не назначен до срабатывания таймера - мы отменим посылку  Дверь-Терминал с помощью метода [отмены](https://developers.avito.ru/api-catalog/delivery-sandbox/documentation#operation/cancelParcel).
3. Курьер не смог забрать посылку.

Ожидаем событие трекинга `CONFIRMED` + `NOT_HANDED_OVER_BY_SENDER`, хотя оно и не приведет к отмене посылки сразу.
На нашей стороне будет таймер (до 6:00 утра следующего дня), до этого момента должно поступить событие трекинга об успешном заборе  (IN_TRANSIT + PARCEL_IS_TAKEN_BY_COURIER_FOR_INTAKE). Если оно не поступит, мы будем считать, что посылку забрать не удалось, и отменим посылку Дверь-Терминал с помощью метода [отмены](https://developers.avito.ru/api-catalog/delivery-sandbox/documentation#operation/cancelParcel).

3. Товар не приняли на ПВЗ (запрещенка)

Курьер оставляет посылку на том Терминале, где не смог ее сдать (продавец заберет сам).

В таком случае мы будем ожидать следующих событий трекинга:
- `IN_TRANSIT_RETURN` \+ `NOT_ACCEPTED_ON_SENDER_TERMINAL`
- `ON_DELIVERY_RETURN` \+ `NOT_ACCEPTED_ON_SENDER_TERMINAL`
- `RETURNED` \+ `NOT_ACCEPTED_ON_SENDER_TERMINAL`

А также вызов метода /order/realAddress в котором будет указан номер ПВЗ, в котором теперь лежит его товар - чтобы он понимал, откуда его забирать.
> [!IMPORTANT]
> Порядок трекингов IN_TRANSIT_RETURN и ON_DELIVERY_RETURN важен и между отправкой этих событий лучше выждать несколько секунд чтобы предотвратить неправильный порядок их обработки.
> Последний трекинг (RETURNED) ожидаем в случае когда продавец уже забрал посылку

4. Курьер не может доставить посылку.

Мы будем ожидать событие трекинга:
- `CONFIRMED` \+ `COURIER_IS_NOT_ABLE_TO_DELIVER`

Посылка при этом в этот момент не отменится. Отмена посылки произойдет на нашей стороне по таймеру (до 6:00 утра следующего дня) либо по инициативе продавца. Для отмены посылки на стороне службы доставки будет использоваться метод [отмены](https://developers.avito.ru/api-catalog/delivery-sandbox/documentation#operation/cancelParcel).

5. Продавец отказался встречать курьера в указанную дату или попросил перенести на другую дату при звонке продавцу до назначения

Мы будем ожидать событие трекинга:
- `CONFIRMED` \+ `SENDER_REFUSED_INTAKE_CONFIRMATION`
Посылка при этом в этот момент не отменится. Отмена посылки произойдет на нашей стороне по таймеру (начало интервала забора) либо по инициативе продавца. Для отмены посылки на стороне службы доставки будет использоваться метод [отмены](https://developers.avito.ru/api-catalog/delivery-sandbox/documentation#operation/cancelParcel).
...
### Теги в Дверь-Терминал посылках
У посылок будет в запросе указаны два тега - `B2C` и `D2T`.
...
Возможные значения:
- `DELIVERY_TO_DOOR` – доставка до двери. Передается для получателя.
- `DELIVERY_TO_PORCH` – доставка до подъезда. Передается для получателя.
- `DELIVERY_FROM_DOOR` – забор от двери. Передается для отправителя в сценарии Дверь-Терминал.
...
`delivery.courier.pickupContact` - контактное лицо для забора посылки курьером. Заполняется для отправителя в сценарии
Дверь-Терминал. Может отличаться от имени отправителя (например, если отправитель – юридическое лицо).

`delivery.courier.pickupContact.name` - ФИО контактного лица для забора посылки.

`delivery.courier.pickupContact.phone` - телефон контактного лица для забора посылки.
...
- `T2D` – посылка сценария Терминал-Дверь.
- `D2T` – посылка сценария Дверь-Терминал.
...
- `COURIER_ASSIGNED` – курьер назначен на забор посылки у отправителя (сценарий Дверь-Терминал).
- `SENDER_REFUSED_INTAKE_CONFIRMATION` - при согласовании забора, отправитель решил что ему неудобно выбранное время и он будет перевывать курьера на другой слот (сценарий Дверь-Терминал)..
- `NOT_HANDED_OVER_BY_SENDER` - отправитель не встретил курьера (сценарий Дверь-Терминал).
- `COURIER_IS_NOT_ABLE_TO_DELIVER` - произошла авария, курьер не сможет забрать посылку у отправителя (сценарий Дверь-Терминал).
- `COURIER_NOT_FOUND` - в выбранную отправителем дату и время не получится осуществить забор (не удалось назначить курьера) (сценарий Дверь-Терминал).
...
- `PARCEL_IS_TAKEN_BY_COURIER_FOR_INTAKE` – курьер забрал посылку у отправителя (сценарий Дверь-Терминал).
...
- `BROKEN_PRODUCT` – повреждение товара, производственный брак или нарушение целостности упаковки.
- `INCORRECT_PRODUCT` – доставленный товар не соответствует заказанному (обнаружен пересорт или подмена в процессе доставки).
...
- `NOT_ACCEPTED_ON_SENDER_TERMINAL` – посылку не приняли на терминале сдачи (сценарий Дверь-Терминал).
...
- `NOT_ACCEPTED_ON_SENDER_TERMINAL` – см. в секции IN_TRANSIT_RETURN.
...
- Смена адреса получателя (сценарий `changeReceiverTerminalOnConfirmed`) позволяет изменить ПВЗ вручения на новый для посылок в статусе `CONFIRMED`. Для кросс-доставочного заказа обновляется ПВЗ на последнем плече.
...
#### Создание посылки в сценарии B2C Дверь-Терминал
Описание работы "B2C Дверь-Терминал" расположено [здесь](https://developers.avito.ru/api-catalog/delivery-sandbox/documentation#info/b2c_dver-terminal_scenariy).

В этом сценарии по одному заказу существуют два отправления: основное (ПВЗ) и курьерское (забор посылки курьером от
двери отправителя). Песочница позволяет создать любое из них. Тег `B2C` обязателен, теги `D2T_PVZ` и `D2T_COURIER`
нельзя передавать одновременно.

Чтобы создать основное (ПВЗ) отправление без курьерского, передавайте в `tags` `["B2C", "D2T_PVZ"]` – в ответе будет ID
ПВЗ-отправления.
...
Чтобы создать заказ сразу с курьерским отправлением, передавайте в `tags` `["B2C", "D2T_COURIER"]` – в ответе будет ID
курьерского отправления.

<details>
<summary>Пример</summary>
```json
{
  "tags": ["B2C", "D2T_COURIER"]
}
```
</details>
